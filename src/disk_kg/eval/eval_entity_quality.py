...
class EntityEvaluator:
    ...
    def _match_entity(self, predicted: Entity, ground_truth: Set[Entity]) -> Tuple[bool, bool]:
        ...
        for gt_entity in ground_truth:
            ...
            # 名称匹配（精确匹配或包含关系）
            if (
                pred_name_norm == gt_name_norm
                or pred_name_norm in gt_name_norm
                or gt_name_norm in pred_name_norm
            ):
                # 检查类型是否正确
                type_correct = (
                    predicted.label == gt_entity.label
                    or predicted.label in gt_entity.label
                    or gt_entity.label in predicted.label
                )
    ...
    def evaluate(self) -> EvaluationMetrics:
        ...
        for idx, item in enumerate(tqdm(self.dataset, desc="Evaluating")):
            ...
            for pred_entity in unique_pred_entities:
                ...
                if not matched:
                    wrong_entities.append({"name": pred_entity.name, "label": pred_entity.label})

            # 找出未被提取的真实实体
            for gt_entity in gt_entities:
                if gt_entity.name not in matched_gt_names:
                    missed_entities.append({"name": gt_entity.name, "label": gt_entity.label})
            ...
            details.append(
                {
                    ...
                    "ground_truth_entities": [
                        {"name": e.name, "label": e.label} for e in gt_entities
                    ],
                    "extracted_entities": [
                        {"name": e.name, "label": e.label} for e in pred_entities
                    ],  # 原始提取结果（未去重）
                    "extracted_entities_unique": [
                        {"name": e.name, "label": e.label} for e in unique_pred_entities
                    ],  # 去重后的提取结果
